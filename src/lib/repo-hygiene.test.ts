...
const FONT_SIGNATURES = new Set(['774f4632', '774f4646', '00010000', '74727565', '4f54544f']);
...
function stripJsonComments(source: string): string {
  return source
    ...
    .replace(JSON_STRING_OR_COMMENT, (_match, literal: string | undefined) => literal ?? '');
}

function parseJsoncFile(repo: Repo, path: string): ParsedFile {
  try {
    return { path, kind: 'parsed', value: JSON.parse(stripJsonComments(readTracked(repo, path))) };
  } catch (error) {
    return {
      path,
      kind: 'unparseable',
...
function parsedValues(files: readonly ParsedFile[]): { path: string; value: unknown }[] {
  return files.flatMap((file) =>
...
function containsFolderOpenTask(value: unknown): boolean {
  ...
  return record['runOn'] === 'folderOpen' || Object.values(record).some(containsFolderOpenTask);
}
...
function allowsAutomaticTasks(value: unknown): boolean {
  ...
  const setting = (value as Record<string, unknown>)['task.allowAutomaticTasks'];
...
function ignoresEnv(gitignore: string): boolean {
  return gitignore.split('\n').some((line) => line.trim().startsWith('.env'));
}
...
if (repo === undefined) {
  ...
  const trackedEnvFiles = repo.files.filter((path) => basename(path) === '.env');
  ...
  describe('repository hygiene guard', () => {
    describe('tracked .vscode/tasks.json', () => {
      ...
      it('declares no task that runs on folder open', () => expect(folderOpenTasks).toEqual([]));
    });
    ...
    describe('tracked .vscode/settings.json', () => {
      ...
      it('does not allow automatic tasks', () => expect(automaticTaskSettings).toEqual([]));
    });

    describe('tracked font files', () => {
      it('begin with a font signature', () => expect(filesWithoutFontSignature).toEqual([]));
    });
